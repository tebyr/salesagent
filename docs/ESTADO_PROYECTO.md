...
| 1.7.0   | 2026-05-04 | 16     | Seguridad sesión (security_config + SessionTimeoutModal + useIdleTimeout). Enriquecimiento contexto agente (priority_clients + top_products). GoalProgress nested. Migración 005. BEST_PRACTICES_WEBHOOK_AI.md. /mantener-docs (7 docs). Comando /agenteenlocal. |
...
**Último commit:** `4e1bf39` — feat: seguridad de sesión + enriquecimiento de contexto + GoalProgress + best practices
...
|--------|---------|--------|-------------------|
| Tenant | `app/models/tenant.py` | ✅ | whatsapp_access_token encriptado. `security_config` JSONB (session_timeout_minutes, session_warning_minutes) |
...
| `005_add_security_config.py` | ✅ | Columna `security_config` JSONB en `tenants`. Default: `{"session_timeout_minutes": 30, "session_warning_minutes": 2}`. |

> ⚠️ Correr siempre `alembic upgrade head` (aplica las 5). Nunca aplicar 001 sola.
...
| goals | `admin/goals.py` | ✅ | CRUD metas. `GoalProgress` nested (actual_amount, pct_amount, projected_amount, actual_visits, actual_new_clients). `salesperson_name` en `GoalOut`. |
| settings | `admin/settings.py` | ✅ | WhatsApp config (token encriptado, access_token opcional); `PUT /security` (session timeout); roles admin+manager. `GET /settings` devuelve `whatsapp_config` + `security_config`. |
...
| AnalyticsService | `services/analytics_service.py` | ✅ | goal_progress, route_data, recommendations, affinities. **`get_salesperson_today_context()`** — enriquece user_info con métricas del día + `priority_clients` (top 8 por días sin compra) + `top_products` (top 5 por revenue 60d). |
...
| `docs/DATA_DICTIONARY.md` | ✅ | v1.9.1 — `security_config` JSONB en tenants + migración 005 |
| `docs/BEST_PRACTICES_WEBHOOK_AI.md` | ✅ | Nuevo — 600 líneas, 10 secciones, modo acumulativo. Lecciones: contexto enriquecido, día de semana, seed freshness |
| `docs/ESTADO_PROYECTO.md` | ✅ | v1.7.0 (este archivo) |
| `docs/ROADMAP.md` | ✅ | v1.6.0 |
...
| `.claude/commands/agenteenlocal.md` | ✅ | Slash command /agenteenlocal — levanta todos los servicios, guía actualización Meta token |
...
# 3. Aplicar migraciones (siempre las 5: 001→002→003→004→005)
...
| 16 | 2026-05-04 | **Seguridad sesión + contexto enriquecido + documentación acumulativa.** `security_config` JSONB en Tenant + migración 005 + `PUT /settings/security` + `SessionTimeoutModal` + `useIdleTimeout`. WhatsApp settings: pre-populate, autofill off, access_token opcional. `AnalyticsService.get_salesperson_today_context()` extendido con `priority_clients` (top 8) y `top_products` (top 5 / 60d). `GoalProgress` nested en `GoalOut` + `salesperson_name`. Fix `clients.py` zone_name. Seed fix semana actual. `docs/BEST_PRACTICES_WEBHOOK_AI.md` (nuevo, 10 secciones). `/mantener-docs` (7 docs actualizados). Comando `/agenteenlocal` creado y probado. | `4e1bf39` |
