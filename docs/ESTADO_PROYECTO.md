...
**Último commit:** `70ce3c9` — docs: actualizar estado del proyecto v1.3.0 *(pendiente commit sesiones 11+12)*
...
| User | `app/models/user.py` | ✅ | password_hash (no hashed_password). Roles: ADMIN/MANAGER/SUPERVISOR/SALESPERSON/AGENT. SAEnum con `native_enum=False`. |
...
| Conversation | `app/models/conversation.py` | ✅ | WhatsAppConversation, MessageLog. Fix sesión 11: `ai_tokens_used` tipo Integer (antes String). |
...
| **Groq como proveedor IA en desarrollo** | Para pruebas locales se usa Groq (tier gratuito, 6K tokens/min) con `llama-3.1-70b-versatile`. Los modelos default en `config.py` apuntan a Groq. Para producción: cambiar `AI_MODEL_*` a `claude-*` y poner `ANTHROPIC_API_KEY`. Las API keys de todos los proveedores son opcionales en config — al menos una debe estar presente en runtime. | `app/core/config.py`, `.env`, `.env.example`, `docs/MONTAJE_LOCAL.md` |
...
### P2 — Bloqueante para cierre montaje local (frente activo sesión 13)

| # | Tarea | Qué hacer |
|---|-------|-----------|
| 15 | **Completar Fase 2 .env** | Pendiente solo `VOYAGE_API_KEY` (dash.voyageai.com). `GROQ_API_KEY` ✅ y `WHATSAPP_APP_SECRET` ✅ ya configurados. |
| 17 | **Fase 4: WhatsApp e2e** | ngrok + configurar webhook en Meta + tenant sandbox + prueba mensaje real |
...
# 1. Configurar variables de entorno (ya listo en .env)
# GROQ_API_KEY, WHATSAPP_APP_SECRET ya configurados
# Pendiente: VOYAGE_API_KEY (dash.voyageai.com)
